...
#define TEXT_HEIGHT           (HEIGHT <= 10 ? 0U : 2U)      // высота, на которой бежит текст (от низа матрицы)
#define LET_WIDTH             (5U)                          // ширина буквы шрифта
#define LET_HEIGHT            (8U)                          // высота буквы шрифта
#define SPACE                 (1U)                          // пробел
#define LETTER_COLOR          (CRGB::Blue)                  // цвет букв по умолчанию
...
void printWeather(uint32_t thisTime, bool onDemand, bool ONflag)
{
#if USE_WEATHER
  if (currentTemp <= -999.0f) {
    if (onDemand) showWarning(CRGB::Red, 4000U, 500U);
    return;
  }
  if (PRINT_WEATHER == 0 && !onDemand) return;
  CRGB letterColor = CRGB::Blue;
  bool needToPrint = false;
  if (PRINT_WEATHER >= 1U) {
    if (thisTime % 60U == 0U) {
      needToPrint = true;
      letterColor = CRGB::Red;
    } else if (thisTime % PRINT_WEATHER == 0U) {
      needToPrint = true;
      letterColor = CRGB::Blue;
    }
  }
