  ...
  if (!sunsetFlag)
  {
    ...
    if (ONflag )
    {
      switch (currentMode)
      {
        ...
        case EFF_CUBE2D:              DYNAMIC_DELAY_TICK { effTimer = millis(); cube2dRoutine();              Eff_Tick (); }  break;  // ( 41U) Кубик Рубика
        ...
        case EFF_PAINTS:              DYNAMIC_DELAY_TICK { effTimer = millis(); OilPaints();                  Eff_Tick (); }  break;  // ( 48U) Масляные краски
        case EFF_MATRIX:              DYNAMIC_DELAY_TICK { effTimer = millis(); matrixRoutine();              Eff_Tick (); }  break;  // ( 49U) Матрица
        case EFF_TWINKLES:            DYNAMIC_DELAY_TICK { effTimer = millis(); twinklesRoutine();            Eff_Tick (); }  break;  // ( 50U) Мерцание
        ...
        case EFF_NIGHTCITY:           HIGH_DELAY_TICK { effTimer = millis(); NightCity();                  Eff_Tick (); }  break;  // ( 58U) Ночной Город
        case EFF_FIRE:                DYNAMIC_DELAY_TICK { effTimer = millis(); fireRoutine(true);            Eff_Tick (); }  break;  // ( 59U) Огонь
        case EFF_FIRE_2012:           DYNAMIC_DELAY_TICK { effTimer = millis(); fire2012again();              Eff_Tick (); }  break;  // ( 60U) Огонь 2012
        case EFF_FIRE_2018:           DYNAMIC_DELAY_TICK { effTimer = millis(); Fire2018_2();                 Eff_Tick (); }  break;  // ( 61U) Огонь 2018
        case EFF_FIRE_2020:           DYNAMIC_DELAY_TICK { effTimer = millis(); fire2020Routine2();           Eff_Tick (); }  break;  // ( 62U) Огонь 2020
        case EFF_FIRE_2021:           LOW_DELAY_TICK { effTimer = millis(); Fire2021Routine();            Eff_Tick (); }  break;  // ( 63U) Огонь 2021
        case EFF_FIREFLY_TOP:         DYNAMIC_DELAY_TICK { effTimer = millis(); MultipleStream5();            Eff_Tick (); }  break;  // ( 64U) Огoнь верховой
        case EFF_FIREFLY:             DYNAMIC_DELAY_TICK { effTimer = millis(); MultipleStream3();            Eff_Tick (); }  break;  // ( 65U) Огoнь парящий
        case EFF_FIRESPARKS:          HIGH_DELAY_TICK { effTimer = millis(); FireSparks();                 Eff_Tick (); }  break;  // ( 66U) Огонь с искрами
        case EFF_COLOR_RAIN:          DYNAMIC_DELAY_TICK { effTimer = millis(); coloredRain();                Eff_Tick (); }  break;  // ( 67U) Осадки
        case EFF_OSCILLATING:         DYNAMIC_DELAY_TICK { effTimer = millis(); oscillatingRoutine();         Eff_Tick (); }  break;  // ( 68U) Осциллятор
        case EFF_CLOUDS:              HIGH_DELAY_TICK { effTimer = millis(); cloudsNoiseRoutine();         Eff_Tick (); }  break;  // ( 69U) Облака
        case EFF_OCEAN:               HIGH_DELAY_TICK { effTimer = millis(); oceanNoiseRoutine();          Eff_Tick (); }  break;  // ( 70U) Океан
        case EFF_OCTOPUS:             DYNAMIC_DELAY_TICK { effTimer = millis(); Octopus();                    Eff_Tick (); }  break;  // ( 71U) Осьминог
        case EFF_RAINBOW_STRIPE:      HIGH_DELAY_TICK { effTimer = millis(); rainbowStripeNoiseRoutine();  Eff_Tick (); }  break;  // ( 72U) Павлин
        case EFF_HOURGLASS:           DYNAMIC_DELAY_TICK { effTimer = millis(); Hourglass();                  Eff_Tick (); }  break;  // ( 73U) Песочные часы
        case EFF_PAINTBALL:           DYNAMIC_DELAY_TICK { effTimer = millis(); lightBallsRoutine();          Eff_Tick (); }  break;  // ( 74U) Пейнтбол
        case EFF_PICASSO:             DYNAMIC_DELAY_TICK { effTimer = millis(); picassoSelector();            Eff_Tick (); }  break;  // ( 75U) Пикассо
        case EFF_PLASMA:              HIGH_DELAY_TICK { effTimer = millis(); plasmaNoiseRoutine();         Eff_Tick (); }  break;  // ( 76U) Плазма
        case EFF_SPIDER:              LOW_DELAY_TICK { effTimer = millis(); spiderRoutine();              Eff_Tick (); }  break;  // ( 77U) Плазменная лампа
        case EFF_PLASMA_WAVES:        SOFT_DELAY_TICK { effTimer = millis(); Plasma_Waves();               Eff_Tick (); }  break;  // ( 78U) Плазменные волны
        case EFF_FLAME:               LOW_DELAY_TICK { effTimer = millis(); execStringsFlame();           Eff_Tick (); }  break;  // ( 79U) Пламя
        case EFF_PLANETEARTH:         SOFT_DELAY_TICK { effTimer = millis(); PlanetEarth();                Eff_Tick (); }  break;  // ( 80U) Планета Земля
        case EFF_BY_EFFECT:           DYNAMIC_DELAY_TICK { effTimer = millis(); ByEffect();                   Eff_Tick (); }  break;  // ( 81U) Побочный эффект
        case EFF_POPCORN:             LOW_DELAY_TICK { effTimer = millis(); popcornRoutine();             Eff_Tick (); }  break;  // ( 82U) Попкорн
        case EFF_PRISMATA:            LOW_DELAY_TICK { effTimer = millis(); PrismataRoutine();            Eff_Tick (); }  break;  // ( 83U) Призмата
        case EFF_ATTRACT:             DYNAMIC_DELAY_TICK { effTimer = millis(); attractRoutine();             Eff_Tick (); }  break;  // ( 84U) Притяжение
        case EFF_LEAPERS:             DYNAMIC_DELAY_TICK { effTimer = millis(); LeapersRoutine();             Eff_Tick (); }  break;  // ( 85U) Пpыгyны
        case EFF_PULSE:               DYNAMIC_DELAY_TICK { effTimer = millis(); pulseRoutine(2U);             Eff_Tick (); }  break;  // ( 86U) Пульс
        case EFF_PULSE_WHITE:         LOW_DELAY_TICK     { effTimer = millis(); pulseRoutine(8U);             Eff_Tick (); }  break;  // ( 87U) Пульс белый
        case EFF_PULSE_RAINBOW:       DYNAMIC_DELAY_TICK { effTimer = millis(); pulseRoutine(4U);             Eff_Tick (); }  break;  // ( 88U) Пульс радужный
        case EFF_RADIAL_WAWE:         DYNAMIC_DELAY_TICK { effTimer = millis(); RadialWave();                 Eff_Tick (); }  break;  // ( 89U) Радиальная волна
        case EFF_RAINBOW_VER:         DYNAMIC_DELAY_TICK { effTimer = millis(); rainbowRoutine();             Eff_Tick (); }  break;  // ( 90U) Радуга
        case EFF_RAINBOW:             HIGH_DELAY_TICK { effTimer = millis(); rainbowNoiseRoutine();        Eff_Tick (); }  break;  // ( 91U) Радуга 3D
        case EFF_RAINBOW_SPOT:        DYNAMIC_DELAY_TICK { effTimer = millis(); RainbowSpot();                Eff_Tick (); }  break;  // ( 92U) Радужное Пятно
        case EFF_RAINBOW_RINGS:       DYNAMIC_DELAY_TICK { effTimer = millis(); RainbowRings();               Eff_Tick (); }  break;  // ( 93U) Радужные кольца
        case EFF_SNAKE:               DYNAMIC_DELAY_TICK { effTimer = millis(); MultipleStream8();            Eff_Tick (); }  break;  // ( 94U) Радужный змей
        case EFF_RAIN:                DYNAMIC_DELAY_TICK { effTimer = millis(); RainRoutine();                Eff_Tick (); }  break;  // ( 95U) Разноцветный дождь
        case EFF_DANDELIONS:          SOFT_DELAY_TICK  { effTimer = millis(); Dandelions();                 Eff_Tick (); }  break;  // ( 96U) Разноцветные одуванчики
        case EFF_RIVERS:              DYNAMIC_DELAY_TICK { effTimer = millis(); BotswanaRivers();             Eff_Tick (); }  break;  // ( 97U) Реки Ботсваны
        case EFF_LIGHTERS:            DYNAMIC_DELAY_TICK { effTimer = millis(); lightersRoutine();            Eff_Tick (); }  break;  // ( 98U) Светлячки
        case EFF_LIGHTER_TRACES:      DYNAMIC_DELAY_TICK { effTimer = millis(); ballsRoutine();               Eff_Tick (); }  break;  // ( 99U) Светлячки со шлейфом
        case EFF_FEATHER_CANDLE:      DYNAMIC_DELAY_TICK { effTimer = millis(); FeatherCandleRoutine();       Eff_Tick (); }  break;  // (100U) Свеча
        case EFF_AURORA:              HIGH_DELAY_TICK { effTimer = millis(); polarRoutine();               Eff_Tick (); }  break;  // (101U) Северное сияние
        case EFF_SERPENTINE:          HIGH_DELAY_TICK { effTimer = millis(); Serpentine();                 Eff_Tick (); }  break;  // (102U) Серпантин
        case EFF_SCANNER:             DYNAMIC_DELAY_TICK { effTimer = millis(); Scanner();                    Eff_Tick (); }  break;  // (103U) Сканер
        case EFF_SINUSOID3:           HIGH_DELAY_TICK { effTimer = millis(); Sinusoid3Routine();           Eff_Tick (); }  break;  // (104U) Синусоид
        case EFF_COLORS:              HIGH_DELAY_TICK { effTimer = millis(); colorsRoutine2();             Eff_Tick (); }  break;  // (105U) Смена цвета
        case EFF_SNOW:                DYNAMIC_DELAY_TICK { effTimer = millis(); Snowfall();                   Eff_Tick (); }  break;  // (106U) Снегопад
        case EFF_SPECTRUM:            DYNAMIC_DELAY_TICK { effTimer = millis(); Spectrum();                   Eff_Tick (); }  break;  // (107U) Спектрум
        case EFF_SPIRO:               LOW_DELAY_TICK { effTimer = millis(); spiroRoutine();               Eff_Tick (); }  break;  // (108U) Спирали
        case EFF_FLOCK:               LOW_DELAY_TICK { effTimer = millis(); flockRoutine(false);          Eff_Tick (); }  break;  // (109U) Стая
        case EFF_FLOCK_N_PR:          LOW_DELAY_TICK { effTimer = millis(); flockRoutine(true);           Eff_Tick (); }  break;  // (110U) Стая и хищник
        case EFF_ARROWS:              DYNAMIC_DELAY_TICK { effTimer = millis(); arrowsRoutine();              Eff_Tick (); }  break;  // (111U) Стрелки
        case EFF_STROBE:              LOW_DELAY_TICK { effTimer = millis(); StrobeAndDiffusion();         Eff_Tick (); }  break;  // (112U) Строб.Хаос.Дифузия
        case EFF_SHADOWS:             LOW_DELAY_TICK { effTimer = millis(); shadowsRoutine();             Eff_Tick (); }  break;  // (113U) Тени
        case EFF_PACIFIC:             LOW_DELAY_TICK { effTimer = millis(); pacificRoutine();             Eff_Tick (); }  break;  // (114U) Тихий океан
        case EFF_TORNADO:             LOW_DELAY_TICK { effTimer = millis(); Tornado();                    Eff_Tick (); }  break;  // (115U) Торнадо
        case EFF_SIMPLE_RAIN:         DYNAMIC_DELAY_TICK { effTimer = millis(); simpleRain();                 Eff_Tick (); }  break;  // (116U) Tyчкa в банке
        case EFF_FIREWORK:            SOFT_DELAY_TICK { effTimer = millis(); Firework();                   Eff_Tick (); }  break;  // (117U) Фейерверк
        case EFF_FIREWORK_2:          DYNAMIC_DELAY_TICK { effTimer = millis(); fireworksRoutine();           Eff_Tick (); }  break;  // (118U) Фейерверк 2
        case EFF_FAIRY:               DYNAMIC_DELAY_TICK { effTimer = millis(); fairyRoutine();               Eff_Tick (); }  break;  // (119U) Фея
        case EFF_FONTAN:              DYNAMIC_DELAY_TICK { effTimer = millis(); Fountain();                   Eff_Tick (); }  break;  // (120U) Фонтан
        case EFF_TRICOLOR:            LOW_DELAY_TICK { effTimer = millis(); FlagRoutine();                Eff_Tick (); }  break;  // (121U) Флаг (Триколор)
        case EFF_COLOR:               LOW_DELAY_TICK  { effTimer = millis(); colorRoutine();               Eff_Tick (); }  break;  // (122U) Цвет
        case EFF_EFF_COLORED_PYTHON : LOW_DELAY_TICK { effTimer = millis(); Colored_Python();             Eff_Tick (); }  break;  // (123U) Цветной Питон
        case EFF_EFF_SAND:            DYNAMIC_DELAY_TICK { effTimer = millis(); sandRoutine();                Eff_Tick (); }  break;  // (124U) Цветные драже
        case EFF_COLOR_FRIZZLES:      SOFT_DELAY_TICK { effTimer = millis(); ColorFrizzles();              Eff_Tick (); }  break;  // (125U) Цветные кудри
        case EFF_EFF_LOTUS:           DYNAMIC_DELAY_TICK { effTimer = millis(); LotusFlower();                Eff_Tick (); }  break;  // (126U) Цветок лотоса
        case EFF_TURBULENCE:          DYNAMIC_DELAY_TICK { effTimer = millis(); Turbulence();                 Eff_Tick (); }  break;  // (127U) Цифровая турбулентность
        case EFF_SPHERES:             LOW_DELAY_TICK { effTimer = millis(); spheresRoutine();             Eff_Tick (); }  break;  // (128U) Шapы
        case EFF_NEXUS:               LOW_DELAY_TICK { effTimer = millis(); nexusRoutine();               Eff_Tick (); }  break;  // (129U) Nexus
        case EFF_EFF_COLOR_FADED:     LOW_DELAY_TICK { effTimer = millis(); colorFaded();                 Eff_Tick (); }  break;  // (130U) Цвет с затуханием
        case EFF_CLOCK:               DYNAMIC_DELAY_TICK { effTimer = millis(); clockRoutine();               Eff_Tick (); }  break;  // (131U) Часы
